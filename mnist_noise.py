...
def test(add_noise, nosie_intensity):  # [0,255]
    print("Testing model")
    ...
    # plt.show()
    # 保存测试结果outp
    sp = pathlib.Path(args.save_path) / f"{diff.save_name()}_outp_{args.noise_intensity}.pt"
    torch.save(outp, sp)
...
def load_outp(diff, load_path):
    try:
        # 构建路径
        lp = pathlib.Path(load_path) / f"{diff.save_name()}_outp_{args.noise_intensity}.pt"
...
if __name__ == "__main__":
    ...
    for label in range(0, 1):
        ...
        for add_noise in range(1, 4):
            # 添加噪声强度的循环
            noise_intensity_range = [0.1 * i for i in range(1, 4)]  # 噪声强度从 0.1 增大到 1.0
            for noise_intensity in noise_intensity_range:

                print(f"Test for add_noise:{add_noise}, intensity{noise_intensity}")
