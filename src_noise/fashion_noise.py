...
def parse_args(args):
    ...
    parser.add_argument(
        "--load-path",
        type=str,
        default="results/formal/fashion_",
    ...
    parser.add_argument("--ds-size", type=int, default=50, help="Dataset size. 80% is used for training.")
    # 所有预模型学习率i
    # u-net
    ...
    parser.add_argument("--batch_size", type=int, default=16, help="Batch size for traning.")
...
if __name__ == "__main__":
    ...
    for label in range(0, 1):
        ...
        # 噪声类型循环
        for add_noise in range(1, 2):
            # 添加噪声强度的循环
            noise_intensity_range = [0, 0.2, 0.5, 0.8]  # 噪声强度从 0.1 增大到 1.0
            for noise_intensity in noise_intensity_range:
                ...
                for model_args in args.model:
                    ...
                    noise_intensity_range = [0.1 * i for i in range(1, 11)]  # 噪声强度从 0.1 增大到 1.0
                ...
                # 显示并保存损失曲线
                show_metrics(loss_dict, "LOSS", "Iteration", args, model_name=model_name, model_params=model_params)
            ...
            show_metrics(ssim_noise_dict, 'SSIM', 'Noise Intensity', args)
            show_metrics(psnr_noise_dict, 'PSNR', 'Noise Intensity', args)
            show_metrics(cos_noise_dict, 'Cosine', 'Noise Intensity', args)
            show_metrics(fid_noise_dict, 'FID', 'Noise Intensity', args)
