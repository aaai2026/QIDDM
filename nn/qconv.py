...
class _QConv2d_FAST(torch.nn.Module):
    ...
    def __init__(
        ...
        self.unfold = torch.nn.Unfold(kernel_size=kernel_size, padding=padding).double()  # Ensure Unfold uses double
        ...
        self.weights = torch.rand(template_shape, dtype=torch.double, requires_grad=True)  # Set dtype to double
    ...
    def _circuit(self, features):
        qml.AmplitudeEmbedding(
            features=features.double(), wires=range(self.wires), pad_with=0.5, normalize=True
        )
        qml.StronglyEntanglingLayers(qw_map.tanh(self.weights.double()), wires=range(self.wires))
    ...
    def forward(self, x):
        ...
        x = self.unfold(x.double())  # Ensure input tensor to Unfold is double
        x = einops.rearrange(x, "batch channel feat -> (batch feat) channel").double()  # Ensure rearranged tensor is double
        x = x + 0.1
        x = self._post_process(x.double())  # Ensure post-processed tensor is double
        x = einops.rearrange(
            x,
            ...
            h_out=h_out,
            w_out=w_out,
        ).double()  # Ensure final rearranged tensor is double
        return x
        ...
        if not mode and self.sample_qnode is None:
            ...
            def _sub_circuit():
                qml.StronglyEntanglingLayers(
                    qw_map.tanh(self.weights.double()), wires=range(self.wires)
                )

            # Add wire_order to the matrix function call
            matrix = qml.matrix(_sub_circuit, wire_order=range(self.wires))()
