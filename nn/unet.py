...
def Conv2d(**kwargs):
    ...
    if qdepth > 0:
        return QConv2d(qdepth=qdepth, **kwargs)  # Remove dtype, convert tensors inside QConv2d
    else:
        return torch.nn.Conv2d(**kwargs).double()  # Ensure Conv2d uses double
...
class UpBlock(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, qdepth=3):
        ...
        ).double()  # Ensure up_conv uses double
        ...
        self.net = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            torch.nn.BatchNorm2d(out_channels, dtype=torch.double),  # Ensure BatchNorm2d uses double
            Conv2d(
            ...
            torch.nn.BatchNorm2d(out_channels, dtype=torch.double),  # Ensure BatchNorm2d uses double
            torch.nn.ReLU(),
        ).double()  # Ensure net uses double

    def forward(self, from_down, from_up):
        from_up = self.up_conv(from_up)
        from_down, from_up = autopad(from_down.double(), from_up.double())  # Ensure autopad uses double
        x = torch.cat([from_up, from_down], dim=1).double()  # Ensure concatenated tensor is double
        x = self.net(x)
        return x
...
class DownBlock(torch.nn.Module):
    ...
    def __init__(self, in_channels, out_channels, pooling, kernel_size=3, qdepth=3):
        ...
        self.net = torch.nn.Sequential(
            ...
            torch.nn.BatchNorm2d(self.out_channels, dtype=torch.double),  # Ensure BatchNorm2d uses double
            torch.nn.ReLU(),
            Conv2d(
    ...
    def forward(self, x):
        x = self.net(x.double())  # Ensure input tensor is double
        before_pool = x
        if self.pooling:
...
class UNetUndirected(torch.nn.Module):
    ...
    def __init__(
        ...
        self.down_blocks = torch.nn.ModuleList(down_blocks).double()  # Ensure down_blocks uses double
        self.up_blocks = torch.nn.ModuleList(up_blocks).double()  # Ensure up_blocks uses double
        self.final_conv = Conv2d(
            ...
            padding=0,
            qdepth=qdepth,
        ).double()  # Ensure final_conv uses double

    def forward(self, x):
        encoder_outputs = []  # list of skip connections
        x = x.double()  # Ensure input tensor is double
        ...
        for i, block in enumerate(self.up_blocks):
            ...
            x = block(skip.double(), x.double())  # Ensure skip and x are double
...
class UnetDirected(UNetUndirected):
    def forward(self, x, y):
        mask = get_label_embedding(y.double(), x.shape[2], x.shape[3])
        masked_x = x.double() + mask
