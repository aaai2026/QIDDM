...
# model
from metrics import *
from nn import QDenseUndirected_old_noise, QNN_A, QNN_noise, differN_noise, QIDDM_PL_noise, UNetUndirected, \
QIDDM_LL_noise
...
def parse_args(args):
    ...
    parser.add_argument(
        ...
        default=[
            ...
            ["QIDDM_LL_noise", 28 * 28, "6", "14", "2"],
            # ["QIDDM_LL", 28 * 28, "8", "6", "2"],
            ["QNN_noise", 28 * 28, "8", "14"],
    ...
    parser.add_argument(
        "--load-path",
        type=str,
        default="results/formal/fmnist_",
    ...
    # 所有预模型学习率
    # u-net
    parser.add_argument("--UNetUndirected_lr", type=float, default=0.01, help="Learning rate.")
    # differn
    parser.add_argument("--differN_noise_lr", type=float, default=0.00914, help="Learning rate.")
    # qdense
    ...
    parser.add_argument("--QIDDM_LL_noise_lr", type=float, default=0.0255, help="Learning rate.")
    # qnn
...
def test(add_noise):  # [0,255]
    ...
    # 对生成图像进行逐步归一化到 [0, 1]
    # `generated_images` 的维度是 (iters, batch, 1, height, width)
    for step in range(generated_images.size(0)):  # 遍历每个去噪步骤
        # 获取当前去噪步骤的生成图像
        current_generated_images = generated_images[step]

        # 对生成图像进行归一化到 [0, 1]
        current_generated_images_min = \
        current_generated_images.reshape(current_generated_images.size(0), -1).min(dim=1, keepdim=True)[0]
        current_generated_images_max = \
        current_generated_images.reshape(current_generated_images.size(0), -1).max(dim=1, keepdim=True)[0]

        # 调整生成图像的最小值和最大值的形状，使其与生成图像兼容
        current_generated_images_min = current_generated_images_min.view(-1, 1, 1, 1)
        current_generated_images_max = current_generated_images_max.view(-1, 1, 1, 1)

        # 将当前生成图像归一化到 [0, 1] 的范围
        generated_images[step] = (current_generated_images - current_generated_images_min) / \
                                 (current_generated_images_max - current_generated_images_min + 1e-7)

    # 如果需要，您可以将归一化后的生成图像乘以 255 转换为 [0, 255]
    # generated_images = generated_images * 255.0
    # generated_images = torch.clamp(generated_images, 0.0, 255.0)
    ...
    for label in range(4, 5):
        ...
        for add_noise in range(0, 1):
            ...
            for model_args in args.model:
                ...
                # 根据模型切换设备cuda和学习率，数据
                if torch.cuda.is_available() and isinstance(net, (QIDDM_PL_noise, QIDDM_LL_noise, QNN_noise)):
                    args.device = "cuda"
                else:
                    args.device = "cpu"
            ...
            show_metrics(loss_dict, "LOSS", model_name=model_name, model_params=model_params, args=args, is_loss=True)

            # Calculate and save SSIM
            ssim_values_dict = get_ssim(generated_images_dict, real_images_dict, args, gen_img_count=5,
                                        real_img_count=80)

            # Calculate and save PSNR
            psnr_values_dict = get_psnr(generated_images_dict, real_images_dict, args, gen_img_count=5,
                                        real_img_count=80)

            # Calculate and save Cosine Similarity
            cos_values_dict = get_cosine_similarity(generated_images_dict, real_images_dict, args, gen_img_count=5,
                                                    real_img_count=80)
            ...
            # print("Last FID values:", fid_values_dict)
