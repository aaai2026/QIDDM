...
# model
from metrics import *
from nn import QDenseUndirected_old_noise, QNN_noise, QNN_noise, differN_noise, QIDDM_PL_noise, UNetUndirected, QIDDM_L
...
def parse_args(args):
    ...
    parser.add_argument(
        ...
        default=[
            ...
            # ["differN_noise", 8, "15", "2"],
            ["differN_noise", 8, "4", "2"],
            # ["differN_noise", 8, "6", "2"],
            ["QDenseUndirected_old_noise", "60", "8"],
            ["QIDDM_PL_noise", 8 * 8, "4", "2", "1"],
            ["QNN_noise", 8 * 8, "4", "2"],
            # ["QNN_noise", 8 * 8, "6", "2"],
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.02,
        help="Specify the intensity to be used for adding noise.(0-1)"
    )
    parser.add_argument(
        "--reduced_size",
        type=float,
    ...
    parser.add_argument("--ds-size", type=int, default=500, help="Dataset size. 80% is used for training.")
    # 所有预模型学习率
    # u-net
    ...
    parser.add_argument("--batch_size", type=int, default=1, help="Batch size for traning.")
...
def train():
    ...
    # 根据模型类型设置训练时的设备和微分方法
    if isinstance(diff.net, (QDenseUndirected_old_noise, differN_noise)):
    ...
    diff.train()
    pbar = tqdm.tqdm(total=args.epochs - start_epoch, ascii="⬛⬜", dynamic_ncols=True)  # ,  ncols=150, nrows=1
...
def test(add_noise, noise_intensity):  # [0,255]
    print("Testing model")
    ...
    diff.net.noise_intensity = noise_intensity
    ...
    # 尝试加载 `outp`，如果加载失败则进行测试
    outp = load_outp(diff, args.load_path)
    if outp is None:
    ...
    # 保存不同加噪强度下的图像
    folder_path = pathlib.Path(args.save_path) / f"{add_noise}"/f"{noise_intensity}"
    folder_path.mkdir(parents=True, exist_ok=True)
    img_path = folder_path / f"{diff.save_name()}.png"
    plt.imsave(img_path, generated_images[-1, 0, 0].cpu().numpy(), cmap="gray")

    # 显示并保存去噪图片
    plt.imshow(outp.cpu().double(), cmap="gray")  #
    plt.axis("off")
    sp = pathlib.Path(args.save_path) / f"{diff.save_name()}_{noise_intensity}.png"
    if not sp.parent.exists():
        sp.parent.mkdir(parents=True)
    plt.savefig(sp)
    # plt.show()
    # 保存测试结果outp
    sp = pathlib.Path(args.save_path) / f"{diff.save_name()}_outp_{noise_intensity}.pt"
    torch.save(outp, sp)

    return generated_images, real_images


def load_outp(diff, load_path):
    try:
        # 构建路径
        lp = pathlib.Path(load_path) / f"{diff.save_name()}_outp_{noise_intensity}.pt"
        print(lp)
        outp_load = torch.load(lp)
        print("Model loaded successfully.\n")
        plt.imshow(outp_load.cpu().double(), cmap="gray")  #
        # plt.show()

        return outp_load

    except FileNotFoundError:
        print("Failed to load outp: File not found.\n")
        return None
...
def load_model(diff, load_path, label):
    ...
    try:
        ...
        if torch.cuda.is_available():
            checkpoint = torch.load(lp)
        else:
            checkpoint = torch.load(lp, map_location=torch.device('cpu'))
...
if __name__ == "__main__":
    ...
    for label in range(0, 1):
        ...
        for model_args in args.model:
            ...
            ssim_noise_dict[model_name] = []
            psnr_noise_dict[model_name] = []
            cos_noise_dict[model_name] = []
            fid_noise_dict[model_name] = []
        ...
        # 噪声类型循环
        for add_noise in range(4, 5):
            # 添加噪声强度的循环
            for model_args in args.model:
                model_name = model_args[0]
                ssim_noise_dict[model_name].clear()
                psnr_noise_dict[model_name].clear()
                cos_noise_dict[model_name].clear()
                fid_noise_dict[model_name].clear()

            noise_intensity_range = [0.01 * i for i in range(0, 10)]  # 噪声强度从 0 增大到 1.0

            # 20个噪声点，画图关掉平滑化，（间隔0.05)
            for noise_intensity in noise_intensity_range:

                print(f"\nTest for add_noise: {add_noise}, intensity {noise_intensity}")
                ...
                for model_args in args.model:
                    ...
                    #
                    # if isinstance(net, QDenseUndirected_old_noise):
                    #     args.target = "noise"
                    # else:
                    #     args.target = "data"

                    # 根据模型切换设备cuda和学习率，数据
                    if torch.cuda.is_available() and isinstance(net, (QIDDM_PL_noise, QNN_noise,)):
                        args.device = "cuda"
                    else:
                        args.device = "cpu"
                    ...
                    # 打印模型以确认
                    print(f"\nInitialized {model_name} with parameters {model_params}, with lr {args.lr}")
                    ...
                    # 为加载outp，切换加载路径
                    args.load_path = noise_load_path + str(add_noise)
                    generated_images, real_images = test(add_noise, noise_intensity)
                    args.load_path = noise_load_path + str(0)
                ...
                ssim_values_dict = get_ssim(generated_images_dict, real_images_dict, args, gen_img_count=1,
                                            real_img_count=2)

                # 计算并保存 PSNR，使用最多 10 张生成图像和 4 张真实图像
                psnr_values_dict = get_psnr(generated_images_dict, real_images_dict, args, gen_img_count=1,
                                            real_img_count=2)

                # 计算并保存余弦相似度，使用最多 10 张生成图像和 4 张真实图像
                cos_values_dict = get_cosine_similarity(generated_images_dict, real_images_dict, args, gen_img_count=1,
                                                        real_img_count=2)
                # #
                # # # 计算并保存 fid，使用最多 10 张生成图像和 4 张真实图像
                fid_values_dict = get_fid(generated_images_dict, real_images_dict, args, gen_img_count=1,
                                          real_img_count=2)

                keys1 = list(ssim_noise_dict.keys())
                keys2 = list(ssim_values_dict.keys())

                for model_name, diff_name in zip(keys1, keys2):
                    ssim_noise_dict[model_name].append(ssim_values_dict[diff_name][-1])
                    psnr_noise_dict[model_name].append(psnr_values_dict[diff_name][-1])
                    cos_noise_dict[model_name].append(cos_values_dict[diff_name][-1])
                    fid_noise_dict[model_name].append(fid_values_dict[diff_name][-1])

            show_metrics(ssim_noise_dict, 'SSIM', xlabel="Rotation Angle error intensity", args=args)
            show_metrics(psnr_noise_dict, 'PSNR', xlabel="Rotation Angle error intensity", args=args)
            show_metrics(cos_noise_dict, 'Cosine', xlabel="Rotation Angle error intensity", args=args)
            show_metrics(fid_noise_dict, 'FID', xlabel="Rotation Angle error intensity", args=args)
